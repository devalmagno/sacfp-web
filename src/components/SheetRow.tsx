...
function SheetRow(props: SheetRowProps) {
  const { teachers, setTeachers, calendar, semester, config } =
    useDataContext();

  const [isDisabled, setIsDisabled] = useState(true);
  const [isGeneratePointsheetDisabled, setIsGeneratePointsheetDisabled] =
    useState(props.sheet.schedules?.length === 0);
  const [course, setCourse] = useState(props.sheet.course);
  const [period, setPeriod] = useState(
    props.sheet.period.split(" ")[0].split("º")[0]
  );
  ...
  return (
    <form className="info--row" onSubmit={updateSheet}>
      ...
      <SavedPopUp setShow={setShowSucessPopUp} show={showSucessPopUp} />
    </form>
  );
}

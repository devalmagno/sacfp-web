...
function PointSheetInfo({ sheet, show }: Props) {
  const { calendar, semester, config } = useDataContext();
  const { setType, setIsPointsheetOpen } = useRenderReplacementContext();

  const schedules = sheet.sheet
    .schedules!.map((sch) => {
      const { day, times } = sch;
  ...
  return (
    <div className="pointsheet-info--container">
      ...
      <span>
        {sheet.sheet.period}º - {sheet.sheet.workload}h/a - {sheet.sheet.course}
      </span>
      <span>{schedules}</span>
      ...
      <div className="pointsheet-models">
        ...
        <div className="models--button" onClick={showNormalPointsheet}>
          <BiSpreadsheet fill="#333A56" size={16} />
          <h3>Visualizar Folha de Ponto Normal</h3>
          <BiRightArrowAlt fill="#333A56" size={16} />
        </div>

        <PointsheetModels sheet={sheet} type="Anteposição" />

        <PointsheetModels sheet={sheet} type="Reposição" />

        <PointsheetEadModel sheet={sheet} />

        <PointsheetComplementModel sheet={sheet} />
      </div>
    </div>
  );
}
